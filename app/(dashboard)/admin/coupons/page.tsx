...
import { Plus, Percent, Copy, Trash2 } from "lucide-react"
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="text-right px-6 py-4 text-sm font-semibold text-gray-500">الكود</th>
                <th className="text-right px-6 py-4 text-sm font-semibold text-gray-500">النوع</th>
                <th className="text-right px-6 py-4 text-sm font-semibold text-gray-500">القيمة</th>
                <th className="text-right px-6 py-4 text-sm font-semibold text-gray-500">الاستخدام</th>
                <th className="text-right px-6 py-4 text-sm font-semibold text-gray-500">الحالة</th>
                <th className="text-right px-6 py-4 text-sm font-semibold text-gray-500">انتهاء الصلاحية</th>
              </tr>
            </thead>
            <tbody>
              {coupons.map((coupon) => {
                ...
                return (
                  <tr key={coupon.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <Badge variant={coupon.type === "PERCENTAGE" ? "info" : "default"}>
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-500">
                        {coupon.expiresAt ? formatDate(coupon.expiresAt) : "غير محدد"}
                      </span>
                    </td>
                  </tr>
                )
