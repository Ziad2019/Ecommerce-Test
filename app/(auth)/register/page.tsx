...
import { register as registerAction, loginWithGoogle } from "@/actions/auth-actions"
    ...
    >
      <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1">انضم إلينا واستمتع بأفضل العروض</p>
        </div>

        {/* Google Register */}
        <form action={loginWithGoogle}>
          <Button type="submit" variant="outline" className="w-full h-12 mb-6" size="lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {password && (
              <div className="mt-2">
                ...
                <p className={`text-xs ${passwordStrength >= 4 ? "text-green-600" : passwordStrength >= 3 ? "text-yellow-600" : "text-red-500"}`}>
                  قوة كلمة المرور: {strengthLabel[passwordStrength]}
                </p>
              </div>
            )}
          </div>
          ...
          <label className="flex items-start gap-2 cursor-pointer">
            ...
            <span className="text-sm text-gray-600">
              أوافق على{" "}
              <Link href="/terms" className="text-indigo-600 hover:underline">الشروط والأحكام</Link>
              {" "}و{" "}
        ...
        <div className="mt-6 space-y-2">
          {["شحن مجاني لأول طلب", "خصومات حصرية للأعضاء", "تتبع طلباتك بسهولة"].map((feature) => (
